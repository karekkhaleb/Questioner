...
import '../../app';
import {
  urlRoot, loginAdmin, createMeetup, createQuestion, urlQuestions,
} from './testUtils';
...
describe('add comment endpoint', () => {
  it('should add a comment to a question', (done) => {
    request.post(`${urlQuestions}/${questionObj.id}/comments/`, {
      json: {
  ...
  it('should tell if the question given does not exist', (done) => {
    request.post(`${urlQuestions}/14785/comments/`, {
      json: {
        // userId: adminObj.user.id,
        // questionId: 4785256,
        comment: 'this is a test comment',
  ...
  it('should warn if the questionId is not an integer', (done) => {
    request.post(`${urlQuestions}/not-a-number/comments/`, {
      json: {
